...
export interface YieldForecastResult {
  crop: CropType;
  currentForecastTonsPerAcre: number;
  changePct: number;
  series: YieldForecastPoint[];
  factors: string[];
}

export interface ResourceLine {
  resource: "Water" | "Fertilizer" | "Pesticides" | "Energy";
  currentUsage: number;
  optimalUsage: number;
  unit: string;
...
export type AlertCategory = "weather" | "crop_health" | "yield" | "resource" | "sensor";
...
export interface LearningInsight {
  summary: string;
  confidenceAdjustment: number; // -1..1, applied to readiness/product confidence
  sampleSize: number;
}
